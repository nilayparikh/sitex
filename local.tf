locals {
  ...
  html_files = merge([
    for config in var.static_site_config : {
      for file in fileset(config.src, "**") : format("%s/%s", config.src, file) => { "config" : config, "file" : file } if !anytrue([for pattern in local.exclude_patterns : can(regex(pattern, file))])
    }
  ]...)
}
